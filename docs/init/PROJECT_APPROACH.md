...
- Un crash después del efecto y antes de registrar su resultado deja el Effect Record en `pending` o `unknown`. Si el proveedor ofrece idempotencia durable, recovery y reconciliación consultan al proveedor cuando es posible y reusan la misma key; retries, replays y webhooks duplicados no crean otra key ni otro efecto efectivo. Si no ofrece idempotencia durable, una consulta sólo permite otro intento cuando es durable, autoritativa y demuestra que el efecto no se ejecutó; si es obsoleta, inconclusa, no existe o no demuestra ese estado, Factory no reintenta automáticamente, mantiene el resultado en `unknown` y exige reconciliación o resolución manual antes de reintentar. `unknown` no satisface un Gate.
